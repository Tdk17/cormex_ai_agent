...
class _CrmDirectoryPageState extends State<CrmDirectoryPage> {
  ...
  Future<void> _load() async {
    ...
    if (workspaceId == null || workspaceId.isEmpty) {
      if (!mounted) return;
      setState(() { _loading = false; _error = 'Selecione uma empresa de trabalho antes de abrir este cadastro.'; });
      return;
    }
    setState(() { _loading = true; _error = null; });
    final result = await _http.cloudFunction(name: _listEndpoint, parameters: <String, dynamic>{'workspaceId': workspaceId, 'limit': 100});
    if (!mounted) return;
    switch (result) {
  ...
  Future<void> _openEditor([Map<String, dynamic>? current]) async {
    ...
    final currentEmailOrWebsite = current == null
        ? ''
        : (_isCustomers ? current['email'] : current['website'])?.toString() ?? '';
    final name = TextEditingController(text: current?['name']?.toString() ?? '');
    final email = TextEditingController(text: currentEmailOrWebsite);
    ...
    final shouldSave = await showDialog<bool>(
      ...
      builder: (BuildContext context) => AlertDialog(
        ...
        content: SizedBox(width: 520, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: <Widget>[
          TextField(controller: name, autofocus: !editing, decoration: InputDecoration(labelText: _isCustomers ? 'Nome *' : 'Nome fantasia *')),
          const SizedBox(height: 12),
          if (_isCustomers) TextField(controller: email, keyboardType: TextInputType.emailAddress, decoration: const InputDecoration(labelText: 'E-mail')) else TextField(controller: extra, decoration: const InputDecoration(labelText: 'Razão social')),
          const SizedBox(height: 12),
          ...
          if (!_isCustomers) ...<Widget>[const SizedBox(height: 12), TextField(controller: email, keyboardType: TextInputType.url, decoration: const InputDecoration(labelText: 'Site'))],
        ]))),
        actions: <Widget>[TextButton(onPressed: () => Navigator.of(context).pop(false), child: const Text('Cancelar')), FilledButton(onPressed: () => Navigator.of(context).pop(true), child: Text(editing ? 'Salvar' : 'Cadastrar'))],
      ),
    );
    if (shouldSave != true || !mounted) { name.dispose(); email.dispose(); phone.dispose(); document.dispose(); extra.dispose(); return; }
    ...
    final payload = <String, dynamic>{'name': trimmedName, 'phone': phone.text.trim(), 'document': document.text.trim()};
    if (_isCustomers) {
      payload['email'] = email.text.trim();
    } else {
      payload['legalName'] = extra.text.trim();
      payload['website'] = email.text.trim();
    }
  ...
  Future<void> _create(Map<String, dynamic> entity) async {
    ...
    final entityType = _isCustomers ? 'customer' : 'account';
    final parameters = <String, dynamic>{'workspaceId': workspaceId, 'clientRequestId': 'crm_${entityType}_${DateTime.now().millisecondsSinceEpoch}', entityType: entity};
  ...
  Future<void> _update(String id, Map<String, dynamic> entity) async {
    ...
    setState(() { _saving = true; _error = null; });
    final entityType = _isCustomers ? 'customer' : 'account';
    final idKey = _isCustomers ? 'customerId' : 'accountId';
    final parameters = <String, dynamic>{'workspaceId': workspaceId, idKey: id, entityType: entity};
  ...
  Widget build(BuildContext context) {
    return Scaffold(body: SafeArea(child: RefreshIndicator(onRefresh: _load, child: ListView(padding: const EdgeInsets.all(24), children: <Widget>[
      Wrap(alignment: WrapAlignment.spaceBetween, crossAxisAlignment: WrapCrossAlignment.center, spacing: 16, runSpacing: 12, children: <Widget>[
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[Text(_title, style: Theme.of(context).textTheme.headlineMedium), const SizedBox(height: 4), Text(_isCustomers ? 'Pessoas e contatos comerciais já convertidos em clientes. Leads continuam separados.' : 'Empresas atendidas pelo workspace. Uma empresa pode ter vários clientes/contatos vinculados.', style: Theme.of(context).textTheme.bodyMedium)]),
      ...
      if (_error != null) ...<Widget>[Card(child: Padding(padding: const EdgeInsets.all(16), child: Row(children: <Widget>[const Icon(Icons.error_outline_rounded), const SizedBox(width: 12), Expanded(child: Text(_error!)), TextButton(onPressed: _load, child: const Text('Tentar novamente'))]))), const SizedBox(height: 16)],
      if (_loading) const Center(child: Padding(padding: EdgeInsets.all(48), child: CircularProgressIndicator())) else if (_items.isEmpty) Card(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 44), child: Column(children: <Widget>[
        Icon(_isCustomers ? Icons.people_outline_rounded : Icons.apartment_rounded, size: 48), const SizedBox(height: 14), Text(_isCustomers ? 'Nenhum cliente cadastrado' : 'Nenhuma empresa cadastrada', style: Theme.of(context).textTheme.titleLarge), const SizedBox(height: 8), Text(_isCustomers ? 'Cadastre o primeiro cliente sem precisar convertê-lo de Leads.' : 'Cadastre a primeira empresa/conta atendida pelo CRM.', textAlign: TextAlign.center),
      ]))) else ..._items.map(_buildItem),
    ]))));
  }
  ...
  Widget _buildItem(Map<String, dynamic> item) {
    ...
    return Card(margin: const EdgeInsets.only(bottom: 10), child: ListTile(onTap: _saving ? null : () => _openEditor(item), leading: CircleAvatar(child: Icon(_isCustomers ? Icons.person_outline : Icons.apartment_outlined)), title: Text(name == null || name.isEmpty ? 'Sem nome' : name), subtitle: secondary == null ? null : Text(secondary), trailing: const Icon(Icons.edit_outlined)));
  }
